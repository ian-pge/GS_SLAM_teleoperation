	...
	float* boxmax)
{
	...
	if (abs(opacities[idx]) < 0.0000000001f)
		return;
	...
	conic_opacity[idx] = { conic.x, conic.y, conic.z, 1.0f / (1.0f + expf(-opacities[idx])) };
...
renderCUDA(
	...
	cudaSurfaceObject_t camera_depth) // Depth rajoutée
{
