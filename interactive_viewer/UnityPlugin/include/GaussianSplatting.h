...
class GaussianSplattingRenderer {
	...
	int _sh_degree = 0; //used when learning 3 is the default value
	bool _fastCulling = true;
	float _scalingModifier = 1.0;

	Vector3f _scenemin = Vector3f(-100.0f, -100.0f, -100.0f); // Adjust as needed
    Vector3f _scenemax = Vector3f(100.0f, 100.0f, 100.0f);    // Adjust as needed
	...
	// Since we don't have the scene bounding box, we set default values
...
public:
	...
	void Preprocess(int context, const std::map<int, Matrix4f>& view_mat, const std::map<int, Matrix4f>& proj_mat, const std::map<int, Vector3f>& position, Vector6f frumstums, float fovy, int width, int height);
	void Render(int context, float* image_cuda, float* depth_cuda, cudaSurfaceObject_t camera_depth_cuda, float fovy, int width, int height);
	void Render(float* image_cuda, float* depth_cuda, Matrix4f view_mat, Matrix4f proj_mat, Vector3f position, Vector6f frumstums, float fovy, int width, int height);
