...
class GaussianModel {
 public:
  ...
  // IPC functions
  void updateIpcTensors();
  void share_tensors_over_ipc(const std::string& host = "127.0.0.1", int port = 65434);
  void send_metadata(const nlohmann::json& metadata_list, const std::string& host, int port);
  std::string ipcHandleToHex(const cudaIpcMemHandle_t& ipc_handle);
 ...
 public:
  ...
  // IPC tensors and handles
  float* xyz_ipc_;
  float* features_dc_ipc_;
  float* scaling_ipc_;
  float* rotation_ipc_;
  float* opacity_ipc_;

  cudaIpcMemHandle_t xyz_ipc_handle_;
  cudaIpcMemHandle_t features_dc_ipc_handle_;
  cudaIpcMemHandle_t scaling_ipc_handle_;
  cudaIpcMemHandle_t rotation_ipc_handle_;
  cudaIpcMemHandle_t opacity_ipc_handle_;

  std::unordered_map<std::string, std::pair<float*, cudaIpcMemHandle_t>> tensors_to_share_;
