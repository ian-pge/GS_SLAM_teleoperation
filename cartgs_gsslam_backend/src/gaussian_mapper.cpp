...
void GaussianMapper::trainForOneIteration() {
...

  {
    ...
    // static auto last_logging_time = std::chrono::steady_clock::now();

    // 1) Only do this if we actually have keyframe 22 in the system
    // if (has_keyframe_22_) {
    //   // 2) Check if keyframe 22 is in the scene
    //   auto itKF = scene_->keyframes().find(22UL);
    //   if (itKF != scene_->keyframes().end()) {
    //     // 3) Compute how long since we received keyframe 22
    //     auto now_time = std::chrono::steady_clock::now();
    //     auto ms_since_kf22 = std::chrono::duration_cast<std::chrono::milliseconds>(
    //                             now_time - time_keyframe_22_received_
    //                         ).count();

    //     // 4) Render & measure keyframe 22
    //     auto pkf22 = itKF->second;
    //     auto render_pkg_22 = GaussianRenderer::render(
    //                             pkf22, pkf22->image_height_, pkf22->image_width_,
    //                             gaussians_, pipe_params_, background_, override_color_);
    //     auto rendered_22 = std::get<0>(render_pkg_22);
    //     auto gt_22       = pkf22->original_image_;

    //     float ssim_val = loss_utils::fast_ssim(rendered_22, gt_22).item().toFloat();
    //     float psnr_val = loss_utils::psnr(rendered_22, gt_22).item().toFloat();

    //     // 5) Open the "quality_vs_it.txt" file.
    //     std::filesystem::path qualityFilePath = result_dir_ / "quality_vs_it.txt";
    //     bool fileExists = std::filesystem::exists(qualityFilePath);
    //     std::ofstream quality_file;
    //     if (!fileExists) {
    //       // File doesn't exist yet: write the header including keyframe 22 pose.
    //       quality_file.open(qualityFilePath.string());
    //       if (quality_file.is_open()) {
    //         quality_file << "Iteration PSNR SSIM Time_since_kf22(ms)\n";
    //       }
    //       quality_file.close();
    //     }
    //     // Now, open in append mode and write the current line.
    //     quality_file.open(qualityFilePath.string(), std::ios::app);
    //     if (quality_file.is_open()) {
    //       quality_file << getIteration() << " "
    //                   << psnr_val       << " "
    //                   << ssim_val       << " "
    //                   << ms_since_kf22  << "\n";
    //     }
    ...
    //     static int last_saved_kf22_iter = 0;
    //     int current_iter = getIteration();
    //     if (current_iter - last_saved_kf22_iter >= 100) {
    //       // Convert the rendered keyframe tensor to an OpenCV image
    //       cv::Mat rendered_img_cv = tensor_utils::torchTensor2CvMat_Float32(rendered_22);
    //       // Convert from RGB to BGR and scale for saving
    //       cv::cvtColor(rendered_img_cv, rendered_img_cv, CV_RGB2BGR);
    //       rendered_img_cv.convertTo(rendered_img_cv, CV_8UC3, 255.0f);
    //       // Create a filename that includes the current iteration
    //       std::filesystem::path kf22_img_path = result_dir_ / ("kf22_" + std::to_string(current_iter) + ".jpg");
    //       cv::imwrite(kf22_img_path.string(), rendered_img_cv);

    //       // Update the last saved iteration number
    //       last_saved_kf22_iter = current_iter;
    //     }
    //   }
    // }
...
void GaussianMapper::handleNewKeyframe(std::tuple<unsigned long /*Id*/,
  ...
  // if (std::get<0>(kf) == 22UL) {
  //   time_keyframe_22_received_ = std::chrono::steady_clock::now();
  //   has_keyframe_22_ = true;
  // }
